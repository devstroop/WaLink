...
func TestBillingPlanCRUD(t *testing.T) {
	db := openTestDB(t)
	list, err := db.ListPlans()
	if err != nil || len(list) < 4 {
		t.Fatalf("seeded plans %v %d", err, len(list))
	}
	got, err := db.GetPlan("free")
	if err != nil || got.ID != "free" {
		t.Fatalf("GetPlan %v %v", got, err)
	}
	limits := got.PlanLimits()
	if limits.DailyMessages != 20 {
		t.Error("PlanLimits failed")
	}
	// create
	newPlan := &PlanRecord{ID: "testplan", Name: "Test", Description: "desc", PriceCents: 100, Interval: "month", Limits: `{"daily_messages":10}`, IsDefault: false}
	if err := db.CreatePlan(newPlan); err != nil {
		t.Fatal(err)
	}
	got, _ = db.GetPlan("testplan")
	if got.Name != "Test" {
		t.Error("CreatePlan failed")
	}
	// update
	newPlan.Name = "Updated"
	if err := db.UpdatePlan(newPlan); err != nil {
		t.Fatal(err)
	}
	got, _ = db.GetPlan("testplan")
	if got.Name != "Updated" {
		t.Error("UpdatePlan failed")
	}
	// delete
	if err := db.DeletePlan("testplan"); err != nil {
		t.Fatal(err)
	}
	_, err = db.GetPlan("testplan")
	if err == nil {
		t.Error("expected error after delete")
	}
	// delete non-existent
	if err := db.DeletePlan("nonexistent"); err == nil {
		t.Error("expected error for nonexistent")
	}
	// update non-existent
	if err := db.UpdatePlan(&PlanRecord{ID: "nonexistent", Name: "x"}); err == nil {
		t.Error("expected error")
	}
	// cannot delete plan with subscription
	if err := db.CreateUser(&UserRecord{ID: "u-bill", Username: "billuser", Email: "b@e.com", PasswordHash: "h", RoleID: "builtin-user", Enabled: true}); err != nil {
		t.Fatal(err)
	}
...
func TestBillingSubscriptionAndUsage(t *testing.T) {
	db := openTestDB(t)
	if err := db.CreateUser(&UserRecord{ID: "u-usage", Username: "usageuser", Email: "u@e.com", PasswordHash: "h", RoleID: "builtin-user", Enabled: true}); err != nil {
		t.Fatal(err)
	}
	// Ensure subscription
	if err := db.EnsureUserSubscription("u-usage", "free"); err != nil {
		t.Fatal(err)
	}
	...
	sub, err := db.GetSubscription("u-usage")
	if err != nil || sub.PlanID != "free" {
	...
	got, _ = db.GetSubscriptionByStripeCustomer("cus_123")
	if got == nil {
		t.Error("GetByStripeCustomer failed")
	}
	// Increment usage
	c, err := db.IncrementUsage("u-usage")
	if err != nil || c != 1 {
		t.Fatalf("Increment %v %d", err, c)
	}
	c, _ = db.IncrementUsage("u-usage")
	if c != 2 {
		t.Errorf("expected 2 got %d", c)
	}
	daily, _ := db.GetDailyUsage("u-usage")
	if daily != 2 {
		t.Errorf("expected 2 got %d", daily)
	}
	// GetDaily for no usage user
	daily, _ = db.GetDailyUsage("nonexistent")
	if daily != 0 {
		t.Error("expected 0")
	}
	// Count accounts
	if err := db.CreateAccount(&AccountRecord{ID: "acc-bill", PhoneNumber: "6000000001", AccountName: "a", DataDir: "/tmp/a", UserID: "u-usage"}); err != nil {
		t.Fatal(err)
	}
	cnt, _ := db.CountUserAccounts("u-usage")
	if cnt != 1 {
		t.Errorf("expected 1 got %d", cnt)
	}
	// GetUserPlanLimits
	limits, planID, err := db.GetUserPlanLimits("u-usage")
	if err != nil || planID != "free" || limits.DailyMessages != 20 {
		t.Errorf("GetUserPlanLimits %v %s %v", limits, planID, err)
	}
	// fallback for nonexistent user
	limits, planID, _ = db.GetUserPlanLimits("nonexistent-user")
	if planID != "free" {
		t.Errorf("expected free for nonexistent got %s", planID)
	}
	// Ensure with empty default
	...
	// ListSubscriptions
	list, _ := db.ListSubscriptions()
	if len(list) < 2 {
		t.Errorf("expected >=2 subs got %d", len(list))
	}
	// GetUsageRange
	today := time.Now().UTC().Format("2006-01-02")
	rng, _ := db.GetUsageRange("u-usage", today, today)
	if len(rng) != 1 || rng[0].Messages != 2 {
		t.Errorf("GetUsageRange failed %+v", rng)
	}
	// GetAllDailyUsage
	all, _ := db.GetAllDailyUsage()
	if len(all) < 1 {
		t.Error("GetAllDailyUsage failed")
	}
	// DeleteSubscription
	if err := db.DeleteSubscription("u-usage"); err != nil {
		t.Fatal(err)
	}
	_, err = db.GetSubscription("u-usage")
	if err == nil {
		t.Error("expected error after delete")
	}
	if err := db.DeleteSubscription("nonexistent"); err == nil {
...
func TestGetPlanNotFound(t *testing.T) {
	db := openTestDB(t)
	_, err := db.GetPlan("nonexistent-plan-xyz")
	if err == nil {
		t.Error("expected error")
	}
}
...
func TestGetSubscriptionNotFound(t *testing.T) {
	db := openTestDB(t)
	_, err := db.GetSubscription("nonexistent-user")
	if err == nil {
		t.Error("expected error")
	}
	_, err = db.GetSubscriptionByStripeID("nonexistent")
	if err == nil {
		t.Error("expected error")
	}
	_, err = db.GetSubscriptionByStripeCustomer("nonexistent")
	if err == nil {
		t.Error("expected error")
	}
}
